...
def apply_s_curve_contrast_numpy(image: np.ndarray, strength: float = 2.0) -> np.ndarray:
    """Apply S-curve contrast using NumPy.
    ...
    result = np.where(x < epsilon, 0, 
                        np.where(x > 1 - epsilon, 1, s_curve))
